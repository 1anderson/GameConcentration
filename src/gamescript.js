         ...
         if(pickedTiles.length<2){
             ...
             if (cc.rectContainsPoint(targetRectangle, location)) {
                 if(pickedTiles.indexOf(target)==-1){
                        target.initWithFile("src/assets/tile_"+target.pictureValue+".png");
...
function checkTiles(){
 ...
 var pause = setTimeout(function(){
     if(pickedTiles[0].pictureValue!=pickedTiles[1].pictureValue){
         pickedTiles[0].initWithFile("src/assets/cover.png");
         pickedTiles[1].initWithFile("src/assets/cover.png");
     }
     else{
...
var MemoryTile = cc.Sprite.extend({
 ctor:function() {
 this._super();
 this.initWithFile("src/assets/cover.png");
